  ...
  useEffect(() => {
    const loadAsset = async () => {
      try {
        ...
        // Lấy dữ liệu của asset theo ID
        const assetData = await contract.methods
          .getAssetDetails(id)
          .call({ from: account.address });

        // Chuyển đổi kiểu dữ liệu, vì các số trong smart contract thường là BigInt
        const asset = {
          name: assetData.name,
      ...
      <div className={`asset-modal-content ${isClosing ? "slide-down" : ""}`}>
        ...
        <div className="asset-content">
          ...
          <div className="asset-image-wrapper">
            <img className="asset-image" src={asset.imageUỦUR} alt={asset.name} />
          </div>
        </div>
      </div>
    </div>
